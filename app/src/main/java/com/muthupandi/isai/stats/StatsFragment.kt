...
class StatsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.statsScroll.setOnApplyWindowInsetsListener { v, insets ->
            v.updatePadding(bottom = insets.systemBarInsetsCompat.bottom + resources.getDimensionPixelSize(R.dimen.spacing_medium))
            insets
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.topSongs.collectLatest { items ->
                ...
                items.take(5).forEachIndexed { index, item ->
                    val itemBinding = ItemStatsTrackBinding.inflate(layoutInflater, binding.statsTopTracksList, true)
                    ...
                    if (index == 0) itemBinding.statsItemRank.setTextColor(0xFFFFD700.toInt()) // Gold
                    if (index == 1) itemBinding.statsItemRank.setTextColor(0xFFC0C0C0.toInt()) // Silver
                    if (index == 2) itemBinding.statsItemRank.setTextColor(0xFFCD7F32.toInt()) // Bronze
                }
            }
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.topArtists.collectLatest { artists ->
                ...
                artists.take(5).forEachIndexed { index, stat ->
                    val itemBinding = ItemStatsGroupBinding.inflate(layoutInflater, binding.statsTopArtistsList, true)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.topAlbums.collectLatest { albums ->
                ...
                albums.take(5).forEachIndexed { index, stat ->
                    val itemBinding = ItemStatsGroupBinding.inflate(layoutInflater, binding.statsTopAlbumsList, true)
                    itemBinding.statsGroupRank.text = "#${index + 1}"

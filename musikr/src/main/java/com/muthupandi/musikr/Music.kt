...
sealed interface Music {
    ...
    private constructor(
        ...
        internal enum class Item(val intCode: Int, val microNamespace: Char) {
            ...
            // part of Isai, so these old integer codes remain.
        ...
        companion object {
            /**
             * Creates an Isai-style [UID] of random composition. Used if there is no
             * non-subjective, unlikely-to-change metadata of the music.
             *
             * @param item The type of [Item] that created this [UID].
             */
            internal fun isai(item: Item): UID {
                return UID(Format.AUXIO, item, UUID.randomUUID())
            }

            /**
             * Creates an Isai-style [UID] with a [UUID] composed of a hash of the non-subjective,
             ...
             *   specification.
             * @return A new isai-style [UID].
             */
            internal fun isai(item: Item, updates: MessageDigest.() -> Unit): UID {
                val digest =

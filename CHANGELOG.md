...
## What's Improved
- Isai can now load 3rd-party SAF folders (ex. RSAF)
  - Note: Third-party foldes are considered by Isai to be more like "SD Cards" internally, so playlist importing may not work well
- Genre subheads are now hidden in the artist view when it is entirely unknown genre
- Isai's playback notification now shows both shuffle/repeat
...
#### What's New
- Basic Tasker integration for safely starting Isai's service
...
#### What's Changed
- Ignore MediaStore tags is now Isai's default and unchangeable behavior. The option has been removed.
...
- Isai has a new, more refined icon style
...
- Massively overhauled how music is loaded [#72]:
	- Isai can now reload music without requiring a restart
...
- Fixed issue where apps like GadgetBridge would not detect Isai
...
## v2.0.0
#### What's New:
- Isai has a new look derived from Material 3
...
- Loading screen has been tweaked to line up with the rest of Isai
